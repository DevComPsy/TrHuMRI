function mri = TrHu_1stL_ppi(mri,analysis_num,coi,voi)

%% for running PPI analysis 

%% additional paths
addpath 'C:\Users\Kenza Kedri\Documents\GitHub\gen_funct'
%% general setup
mri.res_dir(1) = 'C';
ppi_dir = ['E:\Information_gathering-main\Analysis\new\1stL\' analysis_num '_ppi_' coi '_' voi '\'  num2str(mri.ID) '\'];
spm_dir = ['E:\Information_gathering-main\Analysis\new\1stL\big_model_with_totev\'  num2str(mri.ID) '\'];
mkdir(ppi_dir);

%% aggregate beh data
ons =  load(['E:\Information_gathering-main\Analysis\BEH\' num2str(mri.ID) '\' num2str(mri.ID) 'onsets_struct.mat']);
beh =  load(['E:\Information_gathering-main\Analysis\BEH\' num2str(mri.ID) '\' num2str(mri.ID) 'beh_regs.mat']);
...
for m = 1:mri.nblocks
    scans{m,1} = ['E:\Information_gathering-main\Analysis\sub-' num2str(mri.ID) '\func\' mri.epi_dirs{m}];
    ...
    path_to_move_params{m} =  {['E:\Information_gathering-main\Analysis\Physio_regressors\sub-' num2str(mri.ID)...

...
%
function [maskBetas, maskXYZ, maskMNI, subjects] = extractMaskBetas_fast(Mask_file,SPM_dir,Optional_path,verbose)
if nargin < 3
    Optional_path = '';
end
...
for s=1:con_num
    selected_outputs =dirPaths{s};

    try
        subjects(b) = str2double(SPM_files(s).name);
    catch

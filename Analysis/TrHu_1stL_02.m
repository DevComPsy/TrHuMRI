...
%% general setup
res_dir = ['D:\InformationGatheringMRI\derivatives\Analysis\1stL\' analysis_num '\' num2str(mri.ID) '\'];
mkdir(res_dir);
%general functions
...
%% aggregate beh data
ons =  load(['D:\InformationGatheringMRI\BEH\' num2str(mri.ID) '\' num2str(mri.ID) 'onsets_struct.mat']);
beh =  load(['D:\InformationGatheringMRI\BEH\' num2str(mri.ID) '\' num2str(mri.ID) 'beh_regs.mat']);

if strcmp(mri.ID, '102')
   mri.nblocks =3;
end

fun_file = dir([mri.res_dir,'sub-',num2str(mri.ID),'\func\swu*']);
for m = 1:mri.nblocks


    scans{m,1} = [fun_file(m).folder,'/',fun_file(m).name];
    ...
    path_to_move_params{m} =  {['D:\InformationGatheringMRI\derivatives\sub-' num2str(mri.ID)...
...
for b=1:mri.nblocks
    ...
    %trial number
    matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).pmod(2).name = 'trial_num';
    matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).pmod(2).param = zscore(beh.beh_regs(b).trial);
   ...
   % matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).orth = mri.epi_params.orth;
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).duration = 0;
    matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).tmod = 0;
   % %points; earned at the end of each game
    % matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).pmod(1).name = 'points';
    % matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).pmod(1).param = zscore(beh.beh_regs(b).points);
    % matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).pmod(1).poly = 1;     
    % matlabbatch{1}.spm.stats.fmri_spec.sess(b).cond(n).orth = mri.epi_params.orth;
    % 
    %  n=n+1;
...
%% effect of interest
% eoi = eye(mri.nblocks*roi+sum(nui_r));
% for b = 1:mri.nblocks
%     for i = 1:nui_r(b)
%         if b == 1
%             eoi(b*roi+i,b*roi+i) = 0;
%         else
%             eoi(b*roi+sum(nui_r(1:b-1))+i,b*roi+sum(nui_r(1:b-1))+i) = 0;
%         end
%     end
% end
% matlabbatch{4}.stats{1}.con.spmmat = {[res_dir,'SPM.mat']};
% matlabbatch{4}.stats{1}.con.consess{1}.fcon.name = 'effect of interest';
% matlabbatch{4}.stats{1}.con.consess{1}.fcon.convec = {eoi};
% matlabbatch{4}.stats{1}.con.consess{1}.fcon.sessrep = 'none';
% matlabbatch{4}.stats{1}.con.delete = 0;

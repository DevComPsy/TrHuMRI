...
%
function TreasHunt_pract_initialize(userID,debug,pract)
...
params.general.wd_exp_file          = ['29-Mar-2015_seqs_pract' int2str(pract) '.mat'];
...
% no termination, no slider
if pract == 1    
    params.task.exp.n_blocks            = 1;
    params.task.exp.n_games             = 8;
    ...
    params.task.exp.term                = 0;            % different termination conditions enabled
    params.task.exp.confidence          = 0;            % confindence ratings enabled/disabled
    params.task.exp.p_rating_resp       = 0;          % how often a confidence rating will appear
    params.task.exp.p_rating_term       = 0;          % how often a confidence rating will appear
    params.task.term_cols               = [params.general.display.bg_col;params.general.display.bg_col];

    % with termination & slider
elseif pract == 2    
    params.task.exp.n_blocks            = 1;
    params.task.exp.n_games             = 12;
    params.task.exp.n_trials            = 120;
    
    params.task.exp.GoodsAction         = 0;            % games in goods and action space
    params.task.exp.noTreshCond         = 0;            % games whit fixed trial length where no threshold should appear
    params.task.exp.term                = 1;            % different termination conditions enabled
    params.task.exp.confidence          = 1;            % confindence ratings enabled/disabled
    params.task.exp.p_rating_resp       = 1/3;          % how often a confidence rating will appear
    params.task.exp.p_rating_term       = 1/3;          % how often a confidence rating will appear
    params.task.term_cols                = [121 255 82;...          % green
                                            255 132 255];           % pink    matched for luminance
    
else
    error('unknown practice session')
end
...
params.trigger.nulltime             = 0.01;   % 10 ms
